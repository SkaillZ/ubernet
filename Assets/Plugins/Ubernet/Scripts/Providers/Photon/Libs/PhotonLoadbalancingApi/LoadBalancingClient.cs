...
namespace ExitGames.Client.Photon.LoadBalancing
{
    ...
    public class LoadBalancingClient : IPhotonPeerListener
    {
            ...
            set
            {
                if (this.loadBalancingPeer == null || this.loadBalancingPeer.PeerState != PeerStateValue.Disconnected)
                {
                    this.DebugReturn(DebugLevel.WARNING,
                        "Can't set TransportProtocol. Disconnect first! " + ((this.loadBalancingPeer != null)
                            ? "PeerState: " + this.loadBalancingPeer.PeerState
                            : "loadBalancingPeer is null."));
                    return;
                }
        ...
        {
            get { return this.autoJoinLobby; }

            set { this.autoJoinLobby = value; }
        }
        ...
        public int FriendListAge
        {
            get
            {
                return (this.isFetchingFriendList || this.friendListTimestamp == 0)
                    ? 0
                    : Environment.TickCount - this.friendListTimestamp;
            }
        }
        ...
        {
            get { return this.AuthValues != null && !string.IsNullOrEmpty(this.AuthValues.Token); }
        }
            ...
            string[] expectedUsers = null)
        {
            return OpJoinRandomRoom(expectedCustomRoomProperties, expectedMaxPlayers, MatchmakingMode.FillRoom,
                TypedLobby.Default, null, expectedUsers);
        }
        ...
        {
            return this.OpJoinRandomRoom(expectedCustomRoomProperties, expectedMaxPlayers, matchmakingMode,
        ...
        /// <returns>If the operation could be sent currently (requires connection to Master Server).</returns>
        public bool OpJoinRandomRoom(Hashtable expectedCustomRoomProperties, byte expectedMaxPlayers,
            MatchmakingMode matchmakingMode, TypedLobby lobby, string sqlLobbyFilter, string[] expectedUsers = null)
        {
            if (lobby == null)
            {
                lobby = TypedLobby.Default;
        ...
        {
            if (this.CurrentRoom == null || this.Server != ServerConnection.GameServer ||
                this.State == ClientState.DisconnectingFromGameserver)
            {
                return false;
            }
        ...
        public bool OpSetCustomPropertiesOfActor(int actorNr, Hashtable propertiesToSet, Hashtable expectedProperties,
            bool webForward)
        {
            return this.OpSetCustomPropertiesOfActor(actorNr, propertiesToSet, expectedProperties,
                (webForward ? new WebFlags(WebFlags.HttpForwardConst) : null));
        }
            ...
            Hashtable expectedProperties = null, WebFlags webFlags = null)
        {
            if (this.CurrentRoom == null)
            {
                ...
                {
                    this.DebugReturn(DebugLevel.ERROR,
                        "OpSetPropertiesOfActor() failed because this client is not in a room currently. State: " +
                        this.State);
                }
        ...
        /// <summary>Replaced by a newer version with WebFlags.</summary>
        [Obsolete("Use the overload with WebFlags.")]
        public bool OpSetCustomPropertiesOfRoom(Hashtable propertiesToSet, Hashtable expectedProperties,
            bool webForward)
        {
            return this.OpSetCustomPropertiesOfRoom(propertiesToSet, expectedProperties,
                (webForward ? new WebFlags(WebFlags.HttpForwardConst) : null));
        }
            ...
            WebFlags webFlags = null)
        {
            if (this.CurrentRoom == null)
            {
                ...
                {
                    this.DebugReturn(DebugLevel.ERROR,
                        "OpSetPropertiesOfRoom() failed because this client is not in a room currently. State: " +
                        this.State);
                }
        ...
        private void CleanCachedValues()
        {
            ...
            if (this.Server == ServerConnection.GameServer || this.State == ClientState.Disconnecting ||
                this.State == ClientState.PeerCreated)
            {
            ...
            if (this.Server == ServerConnection.MasterServer || this.State == ClientState.Disconnecting ||
                this.State == ClientState.PeerCreated)
            {
        ...
        public virtual void OnStatusChanged(StatusCode statusCode)
        {
            switch (statusCode)
            {
                ...
                case StatusCode.EncryptionEstablished:
                    ...
                    if (this.Server != ServerConnection.NameServer &&
                        (this.AuthMode == AuthModeOption.AuthOnce || this.AuthMode == AuthModeOption.AuthOnceWss))
                    {
